  ...
  private readonly logger = new Logger(HttpExceptionFilter.name);
    ...
    // 处理TypeORM查询失败异常
      ...
      if (!isHealthCheck) {
        ...
        this.logger.error(`参数: ${(exception as any).parameters ? JSON.stringify((exception as any).parameters) : 'none'}`);
      }
          ...
          try {
            ...
            for (const key in queryParams) {
              ...
              if (typeof value === 'string' && value.match(/^\d{4}-\d{2}-\d{2}/)) {
                ...
                this.logger.error(`转换后: ${date}, isNaN: ${isNaN(date.getTime())}`);
              }
              ...
              if (typeof value === 'string' && key.toLowerCase().includes('id')) {
                ...
                this.logger.error(`转换后: ${numValue}, isNaN: ${isNaN(numValue)}`);
              }
            }

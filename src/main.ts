...
import { DocumentBuilder, SwaggerModule } from './common/swagger'; // 这是生成API文档的工具
...
async function bootstrap() {
  ...
  const app = await NestFactory.create(AppModule, {
    // 根据环境设置不同的日志级别
    logger: process.env.LOG_LEVEL === 'debug' ? logLevels :  // 调试模式：显示所有日志
            process.env.LOG_LEVEL === 'info' ? logLevels.slice(0, 3) : // 信息模式：显示错误、警告、普通日志
            process.env.LOG_LEVEL === 'warn' ? logLevels.slice(0, 2) : // 警告模式：只显示错误和警告
            ['error'], // 默认模式：只显示错误
  });

...
export class Attendance {
  ...
  @Column({ type: 'tinyint', unsigned: true, nullable: true, comment: '记录类型：1-固定上下班；2-外出；3-按班次上下班；4-自由签到；5-加班；7-无规则' })
  record_type: number;
  ...
  @Column({ type: 'tinyint', unsigned: true, nullable: true, comment: '日报类型：0-工作日日报；1-休息日日报' })
  day_type: number;
  ...
  @Column({ type: 'decimal', precision: 5, scale: 2, nullable: true, comment: '当日实际工作时长，单位：小时' })
  regular_work_sec: number;

  @Column({ type: 'decimal', precision: 5, scale: 2, nullable: true, comment: '当日标准工作时长，单位：小时' })
  ...
  @Column({ type: 'tinyint', unsigned: true, default: 0, comment: '地点异常次数' })
  exception_location: number;

  @Column({ type: 'tinyint', unsigned: true, default: 0, comment: '设备异常次数' })
  ...
  // 加班信息
  @Column({ type: 'tinyint', unsigned: true, default: 0, comment: '加班状态：0-无加班；1-正常；2-缺时长' })
  ot_status: number;

  @Column({ type: 'decimal', precision: 5, scale: 2, default: 0, comment: '加班时长(小时)' })
  ot_duration: number;

  @Column({ type: 'decimal', precision: 5, scale: 2, default: 0, comment: '加班不足的时长(小时)' })
  exception_duration: number;

  @Column({ type: 'decimal', precision: 5, scale: 2, default: 0, comment: '工作日加班记为调休(小时)' })
  workday_over_as_vacation: number;

  @Column({ type: 'decimal', precision: 5, scale: 2, default: 0, comment: '工作日加班记为加班费(小时)' })
  workday_over_as_money: number;

  @Column({ type: 'decimal', precision: 5, scale: 2, default: 0, comment: '休息日加班记为调休(小时)' })
  restday_over_as_vacation: number;

  @Column({ type: 'decimal', precision: 5, scale: 2, default: 0, comment: '休息日加班记为加班费(小时)' })
  restday_over_as_money: number;

  @Column({ type: 'decimal', precision: 5, scale: 2, default: 0, comment: '节假日加班记为调休(小时)' })
  holiday_over_as_vacation: number;

  @Column({ type: 'decimal', precision: 5, scale: 2, default: 0, comment: '节假日加班记为加班费(小时)' })

...
import { Repository, Between, Like, MoreThanOrEqual, LessThanOrEqual, Raw } from 'typeorm';
...
export class AttendanceService {
  ...
  async create(createDto: CreateAttendanceDto): Promise<Attendance> {
    try {
      ...
      if (createDto.checkintime_work) {
        const [hours, minutes] = createDto.checkintime_work.split(':').map(Number);
      ...
      if (createDto.checkintime_off_work) {
        const [hours, minutes] = createDto.checkintime_off_work.split(':').map(Number);
  ...
  async update(updateDto: UpdateAttendanceDto): Promise<Attendance> {
    try {
      ...
      // 处理时间字符串
      if (updateData.checkintime_work && typeof updateData.checkintime_work === 'string') {
        const [hours, minutes] = updateData.checkintime_work.split(':').map(Number);
        const workTime = new Date(0);
        workTime.setUTCHours(hours, minutes, 0);
        // 转换类型
      ...
      if (updateData.checkintime_off_work && typeof updateData.checkintime_off_work === 'string') {
        const [hours, minutes] = updateData.checkintime_off_work.split(':').map(Number);
        const offWorkTime = new Date(0);
        offWorkTime.setUTCHours(hours, minutes, 0);
        // 转换类型
   ...
   * @param queryDto 查询参数
   */
  async findAll(queryDto: QueryAttendanceDto): Promise<{ items: Attendance[]; total: number }> {
    try {
      const { 
        page = 1, 
        pageSize = 10, 
      ...
      // 异常查询
      if (hasException === 1) {
        if (exceptionType !== undefined && exceptionType !== AttendanceExceptionType.ALL) {
          // 根据指定的异常类型查询
      ...
      const validSortFields = [
        'date', 'name', 'departs_name', 'checkin_count', 
        'regular_work_sec', 'standard_work_sec', 'ot_duration',
        'exception_late', 'exception_early', 'exception_absent', 'exception_missing'
      ];
  ...
  async saveAttendanceData(data: any): Promise<Attendance> {
    try {
      // 基本信息检查
      if (!data || !data.base_info || !data.base_info.date || !data.base_info.acctid) {
  ...
  private transformAttendanceData(data: any): Partial<Attendance> {
    try {
      ...
      if (ruleInfo.checkintime && ruleInfo.checkintime.length > 0) {
        if (ruleInfo.checkintime[0] && typeof ruleInfo.checkintime[0].work_sec === 'number') {
        ...
        if (ruleInfo.checkintime[lastIndex] && typeof ruleInfo.checkintime[lastIndex].off_work_sec === 'number') {
      ...
      if (data.exception_infos && Array.isArray(data.exception_infos)) {
        data.exception_infos.forEach(item => {
          ...
          switch(item.exception) {
            case 1: // 迟到
              exceptions['exception_late'] = typeof item.count === 'number' ? item.count : 0;
              // 秒转分钟，向上取整
              exceptions['late_duration'] = typeof item.duration === 'number' ? Math.ceil(item.duration / 60) : 0;
              break;
            case 2: // 早退
              exceptions['exception_early'] = typeof item.count === 'number' ? item.count : 0;
              // 秒转分钟，向上取整
              exceptions['early_duration'] = typeof item.duration === 'number' ? Math.ceil(item.duration / 60) : 0;
              break;
            case 3: // 缺卡
              exceptions['exception_absent'] = typeof item.count === 'number' ? item.count : 0;
              break;
            case 4: // 旷工
              exceptions['exception_missing'] = typeof item.count === 'number' ? item.count : 0;
              // 秒转分钟，向上取整
              exceptions['missing_duration'] = typeof item.duration === 'number' ? Math.ceil(item.duration / 60) : 0;
              break;
            case 5: // 地点异常
              exceptions['exception_location'] = typeof item.count === 'number' ? item.count : 0;
              break;
            case 6: // 设备异常
              exceptions['exception_device'] = typeof item.count === 'number' ? item.count : 0;
              break;
          }
        });
      }
      ...
      try {
        if (typeof summaryInfo.earliest_time === 'number' && !isNaN(summaryInfo.earliest_time)) {
      ...
      try {
        if (typeof summaryInfo.lastest_time === 'number' && !isNaN(summaryInfo.lastest_time)) {
      ...
      if (data.holiday_infos && Array.isArray(data.holiday_infos) && data.holiday_infos.length > 0) {
        const holiday = data.holiday_infos[0];
        
        if (holiday && holiday.sp_title && holiday.sp_title.data && Array.isArray(holiday.sp_title.data) && holiday.sp_title.data.length > 0) {
          spTitle = holiday.sp_title.data[0].text || '';
        }
        
        if (holiday && holiday.sp_description && holiday.sp_description.data && Array.isArray(holiday.sp_description.data) && holiday.sp_description.data.length > 0) {
      ...
      return {
        // 基础信息
        date: date,
        record_type: typeof baseInfo.record_type === 'number' ? baseInfo.record_type : null,
        name: baseInfo.name || '',
        ...
        scheduleid: typeof ruleInfo.scheduleid === 'number' ? ruleInfo.scheduleid : null,
        schedulename: ruleInfo.schedulename || '',
        checkintime_work: checkintime_work,
        checkintime_off_work: checkintime_off_work,
        day_type: typeof baseInfo.day_type === 'number' ? baseInfo.day_type : null,
        
        // 汇总信息
        checkin_count: typeof summaryInfo.checkin_count === 'number' ? summaryInfo.checkin_count : null,
        ...
        workday_over_as_vacation: secondsToHours(otInfo.workday_over_as_vacation),
        workday_over_as_money: secondsToHours(otInfo.workday_over_as_money),
        restday_over_as_vacation: secondsToHours(otInfo.restday_over_as_vacation),
        restday_over_as_money: secondsToHours(otInfo.restday_over_as_money),
        holiday_over_as_vacation: secondsToHours(otInfo.holiday_over_as_vacation),
        holiday_over_as_money: secondsToHours(otInfo.holiday_over_as_money),
        
        // 假勤信息
        sp_number: data.holiday_infos && data.holiday_infos.length > 0 ? data.holiday_infos[0].sp_number : null,
        sp_title: spTitle,

...
export class FinancialSelfInspectionController {
  ...
  @ApiOperation({
    ...
    description:
      '对于普通用户，创建账务自查记录时会进行职级检查：1.如果记账会计是当前用户自己，允许创建；2.如果当前用户职级高于记账会计，允许创建；3.如果双方都是P4职级，允许创建；4.其他情况不允许创建。管理员和超级管理员可以直接创建任意记录，无需职级检查。',
  })
  create(
    ...
    // 传递当前用户名和管理员标识
    return this.financialSelfInspectionService.create(
      createDto,
      req.user.username,
      isAdmin,
    );
  ...
  findMyReviewed(
    ...
    const roles = req.user.roles || [];
    const isAdmin = roles.includes('admin') || roles.includes('super_admin');
    return this.financialSelfInspectionService.findMyReviewed(
      username,
      queryDto,
      isAdmin,
    );
  ...
  findMyReviewedDetail(@Param('id') id: string, @Request() req) {
    ...
    const roles = req.user.roles || [];
    const isAdmin = roles.includes('admin') || roles.includes('super_admin');
    return this.financialSelfInspectionService.findMyReviewedOne(
      +id,
      username,
      isAdmin,
    );
  ...
  async updateRectificationCompletion(
    ...
    this.logger.debug(
      `接收到整改记录: ${JSON.stringify(dto.rectificationRecords)}`,
    );
  ...
  @ApiParam({ name: 'id', description: '记录ID' })
  async updateApproval(@Param('id') id: string, @Body() dto: ApprovalDto) {
    this.logger.debug(
      `接收到审核通过记录: ${JSON.stringify(dto.approvalRecords)}`,
    );
  ...
  async updateReviewerApproval(
    ...
    this.logger.debug(
      `接收到复查审核通过记录: ${JSON.stringify(dto.reviewerApprovalRecords)}`,
    );

    // 确保DTO中的数据结构正确
    if (
      !dto.reviewerApprovalRecords ||
      !Array.isArray(dto.reviewerApprovalRecords)
    ) {
      throw new BadRequestException('复查审核通过记录必须是数组');
    }

    const username = req.user.username;
    return this.financialSelfInspectionService.updateReviewerApproval(
      +id,
      dto,
      username,
    );
  ...
  @ApiParam({ name: 'id', description: '记录ID' })
  async updateReject(@Param('id') id: string, @Body() dto: RejectDto) {
    this.logger.debug(
      `接收到审核退回记录: ${JSON.stringify(dto.rejectRecords)}`,
    );
  ...
  async updateReviewerReject(
    ...
    this.logger.debug(
      `接收到复查审核退回记录: ${JSON.stringify(dto.reviewerRejectRecords)}`,
    );

    // 确保DTO中的数据结构正确
    if (
      !dto.reviewerRejectRecords ||
      !Array.isArray(dto.reviewerRejectRecords)
    ) {
      throw new BadRequestException('复查审核退回记录必须是数组');
    }

    const username = req.user.username;
    return this.financialSelfInspectionService.updateReviewerReject(
      +id,
      dto,
      username,
    );

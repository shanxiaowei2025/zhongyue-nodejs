...
export class TaxVerificationService {
  ...
  async create(createDto: CreateTaxVerificationDto, username: string) {
    ...
    const savedRecord =
      await this.taxVerificationRepository.save(taxVerification);

    // 添加调试信息
    console.log(
      '保存后的attachments数据:',
      JSON.stringify(savedRecord.attachments),
    );
  ...
  async findAll(queryDto: QueryTaxVerificationDto, user: any) {
    ...
    if (filters.unifiedSocialCreditCode) {
      where.unifiedSocialCreditCode = Like(
    ...
    if (filters.riskIssuedDateStart && filters.riskIssuedDateEnd) {
      ...
      where.riskIssuedDate = MoreThanOrEqual(
        new Date(filters.riskIssuedDateStart),
      );
    } else if (filters.riskIssuedDateEnd) {
      where.riskIssuedDate = LessThanOrEqual(
        new Date(filters.riskIssuedDateEnd),
      );
    }
    ...
    const isAccountant =
      userRoles.includes('bookkeepingAccountant') ||
      userRoles.includes('consultantAccountant');
    ...
    // 记账会计和顾问会计只能查看自己负责的企业
      ...
      // 获取统一社会信用代码列表
      const creditCodes = userCustomers
        .map((c) => c.unifiedSocialCreditCode)
        .filter(Boolean);
  ...
  async findOne(id: number, user: any) {
    const record = await this.taxVerificationRepository.findOne({
      where: { id },
    });

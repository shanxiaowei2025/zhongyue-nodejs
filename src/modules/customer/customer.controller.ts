...
export class CustomerController {
  ...
  create(@Body() createCustomerDto: CreateCustomerDto, @Request() req) {
    this.logger.debug(
      `用户请求创建客户，用户信息: ${JSON.stringify(req.user)}`,
    );
  ...
  @ApiOperation({
    summary: '导出客户数据为CSV',
    description:
      '导出符合筛选条件的客户数据。使用startDate和endDate参数可以按创建日期范围筛选，如果使用相同的日期（如startDate=2023-01-01&endDate=2023-01-01），将导出该整天的数据。',
  })
  ...
  async exportToCsv(
    ...
    const hasExportPermission =
      await this.customerPermissionService.hasCustomerExportPermission(
        req.user.id,
      );
  ...
  async importExcel(@UploadedFile() file: Express.Multer.File, @Request() req) {
    ...
    try {
      // 记录文件信息以便调试
      this.logger.log(
        `用户 ${req.user.id} 开始导入客户数据, 文件信息: ${JSON.stringify({
      ...
      if (fileExt !== '.xlsx' && fileExt !== '.xls' && fileExt !== '.csv') {
        throw new BadRequestException(
          '文件格式不支持，请上传Excel(.xlsx/.xls)或CSV(.csv)文件',
        );
      }
  ...
  @ApiResponse({ status: 400, description: '请求参数错误' })
  @ApiResponse({ status: 403, description: '没有权限执行此操作' })
  async updateExcel(@UploadedFile() file: Express.Multer.File, @Request() req) {
    if (!file) {
      throw new ForbiddenException('未提供文件');
    }
    ...
    try {
      // 记录文件信息以便调试
      this.logger.log(
        `用户 ${req.user.id} 开始批量更新客户数据, 文件信息: ${JSON.stringify({
          fieldname: file.fieldname,
          originalname: file.originalname,
          mimetype: file.mimetype,
          size: file.size,
      ...
      const result = await this.customerService.updateCustomers(
        file,
        req.user.id,
      );
  ...
  @ApiOperation({
    summary: '查询客户档案信息',
    description:
      '根据企业名称或统一社会信用代码查询客户的档案相关信息，支持模糊查询。必须提供企业名称或统一社会信用代码中的至少一个参数（公开接口）',
  })
  @ApiResponse({
    status: 200,
    description: '查询成功',

...
@ApiTags('薪资管理补贴合计')
@ApiBearerAuth()
@Controller('salary/subsidy-summary')
...
export class SubsidySummaryController {
    ...
    schema: {
      ...
      properties: {
        file: {
          type: 'string',
          format: 'binary',
          description: 'CSV或Excel文件，包含以下列：姓名、部门、职位、部门负责人补贴、岗位津贴、油补、餐补8元/天、补贴合计(可选)、年月'
        }
      }
    }
  })
  ...
  update(@Param('id') id: string, @Body() updateSubsidySummaryDto: UpdateSubsidySummaryDto) {
    const safeId = safeIdParam(id);
    if (safeId === null) {
      return { success: false, message: '无效的ID参数' };
    }

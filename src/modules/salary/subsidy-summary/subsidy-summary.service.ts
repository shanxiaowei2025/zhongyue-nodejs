...
export class SubsidySummaryService {
  ...
  async create(createSubsidySummaryDto: CreateSubsidySummaryDto): Promise<SubsidySummary> {
    // 自动计算补贴合计
    createSubsidySummaryDto.totalSubsidy = this.calculateTotalSubsidy(createSubsidySummaryDto);
    
    const subsidySummary = this.subsidySummaryRepository.create(createSubsidySummaryDto);
  ...
  async importData(file: Express.Multer.File) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // Python脚本路径
        const scriptPath = join(rootDir, 'src/modules/salary/subsidy-summary/utils/import_subsidy.py');
  ...
  async findAll(query: QuerySubsidySummaryDto) {
    ...
    const queryBuilder = this.subsidySummaryRepository.createQueryBuilder('subsidySummary');
    
    // 字符串字段模糊查询
    if (name) {
      queryBuilder.andWhere('subsidySummary.name LIKE :name', { name: `%${name}%` });
    }
    
    if (department) {
      queryBuilder.andWhere('subsidySummary.department LIKE :department', { department: `%${department}%` });
    }
    
    if (position) {
      queryBuilder.andWhere('subsidySummary.position LIKE :position', { position: `%${position}%` });
    }
    ...
    try {
      ...
      if (safeStartDate && safeEndDate) {
        queryBuilder.andWhere('subsidySummary.yearMonth BETWEEN :startDate AND :endDate', { 
  ...
  async update(id: number, updateSubsidySummaryDto: UpdateSubsidySummaryDto): Promise<SubsidySummary> {
    // 确保id是有效的数字
    const safeId = Number(id);
    if (isNaN(safeId)) {
    ...
    if (
      ...
      if (existingData) {
        // 合并现有数据和更新数据
        const mergedData = {
          departmentHeadSubsidy: updateSubsidySummaryDto.departmentHeadSubsidy ?? existingData.departmentHeadSubsidy,
          positionAllowance: updateSubsidySummaryDto.positionAllowance ?? existingData.positionAllowance,
          oilSubsidy: updateSubsidySummaryDto.oilSubsidy ?? existingData.oilSubsidy,
          mealSubsidy: updateSubsidySummaryDto.mealSubsidy ?? existingData.mealSubsidy,
        };
        // 计算新的合计值
        updateSubsidySummaryDto.totalSubsidy = this.calculateTotalSubsidy(mergedData);
      }
    }
  ...
  private calculateTotalSubsidy(data: {
    ...
    // 确保所有值都转换为数字类型
    const departmentHeadSubsidy = data.departmentHeadSubsidy ? Number(data.departmentHeadSubsidy) : 0;
    const positionAllowance = data.positionAllowance ? Number(data.positionAllowance) : 0;
    const oilSubsidy = data.oilSubsidy ? Number(data.oilSubsidy) : 0;
    const mealSubsidy = data.mealSubsidy ? Number(data.mealSubsidy) : 0;
    
    // 检查是否有任何值不是有效数字
    if (isNaN(departmentHeadSubsidy) || isNaN(positionAllowance) || isNaN(oilSubsidy) || isNaN(mealSubsidy)) {
    ...
    // 计算总和并保留两位小数
    const total = departmentHeadSubsidy + positionAllowance + oilSubsidy + mealSubsidy;

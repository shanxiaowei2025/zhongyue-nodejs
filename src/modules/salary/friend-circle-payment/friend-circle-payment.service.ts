...
export class FriendCirclePaymentService {
  ...
  async create(createFriendCirclePaymentDto: CreateFriendCirclePaymentDto) {
    ...
    if (existing) {
      throw new BadRequestException(
        `${createFriendCirclePaymentDto.name} 在 ${createFriendCirclePaymentDto.yearMonth} 已有记录`,
      );
    }
    ...
    if (
      createFriendCirclePaymentDto.totalCount &&
      createFriendCirclePaymentDto.totalCount !== totalFromWeeks
    ) {
      throw new BadRequestException(
        `总数 (${createFriendCirclePaymentDto.totalCount}) 与每周数据总和 (${totalFromWeeks}) 不一致`,
      );
    }
    ...
    const friendCirclePayment = this.friendCirclePaymentRepo.create(
  ...
  async findAll(query: QueryFriendCirclePaymentDto, req: Request) {
    ...
    const queryBuilder = this.friendCirclePaymentRepo.createQueryBuilder(
      'friendCirclePayment',
    );

    // 姓名模糊查询
    if (name) {
      queryBuilder.andWhere('friendCirclePayment.name LIKE :name', {
        name: `%${name}%`,
      });
    }
    ...
    if (isCompleted !== undefined) {
      ...
      queryBuilder.andWhere('friendCirclePayment.isCompleted = :isCompleted', {
    ...
    if (yearMonth) {
      ...
      if (safeYearMonth) {
        ...
        queryBuilder.andWhere(
          'DATE_FORMAT(friendCirclePayment.yearMonth, "%Y-%m") LIKE :yearMonth',
  ...
  async update(
    id: number,
    updateFriendCirclePaymentDto: UpdateFriendCirclePaymentDto,
  ) {
    ...
    if (
      ...
      const weekOne =
        updateFriendCirclePaymentDto.weekOne ?? friendCirclePayment.weekOne;
      const weekTwo =
        updateFriendCirclePaymentDto.weekTwo ?? friendCirclePayment.weekTwo;
      const weekThree =
        updateFriendCirclePaymentDto.weekThree ?? friendCirclePayment.weekThree;
      const weekFour =
        updateFriendCirclePaymentDto.weekFour ?? friendCirclePayment.weekFour;

      const totalFromWeeks =
        (weekOne || 0) + (weekTwo || 0) + (weekThree || 0) + (weekFour || 0);

      if (
        updateFriendCirclePaymentDto.totalCount !== undefined &&
        updateFriendCirclePaymentDto.totalCount !== totalFromWeeks
      ) {
        throw new BadRequestException(
          `总数 (${updateFriendCirclePaymentDto.totalCount}) 与每周数据总和 (${totalFromWeeks}) 不一致`,
        );
      }
    ...
    // 检查是否已存在相同姓名和年月的记录
    if (
      updateFriendCirclePaymentDto.name !== undefined &&
      updateFriendCirclePaymentDto.yearMonth !== undefined &&
      (updateFriendCirclePaymentDto.name !== friendCirclePayment.name ||
        updateFriendCirclePaymentDto.yearMonth !==
          friendCirclePayment.yearMonth)
    ) {
      const existing = await this.friendCirclePaymentRepo.findOne({
        where: {
      ...
      if (existing && existing.id !== id) {
        throw new BadRequestException(
          `${updateFriendCirclePaymentDto.name} 在 ${updateFriendCirclePaymentDto.yearMonth} 已有记录`,
        );
      }
    }
  ...
  async importData(file: Express.Multer.File) {
    try {
      ...
      const scriptPath = path.join(
        rootDir,
        'src/modules/salary/friend-circle-payment/utils/import_payment.py',
      );
      ...
      return new Promise<any>((resolve, reject) => {
          ...
          try {
            ...
            if (importedCount > 0) {
              ...
              if (overwriteMode) {
                for (const item of result.data) {
                  if (item.name && item.yearMonth) {
                    // 提取年月信息（YYYY-MM格式）
                    const yearMonthStr = item.yearMonth
                      .toString()
                      .substring(0, 7);
                    ...
                    await this.friendCirclePaymentRepo
                      ...
                      .andWhere(
                        'DATE_FORMAT(yearMonth, "%Y-%m") = :yearMonth',
              ...
              const entities = result.data.map((item) =>
                this.friendCirclePaymentRepo.create(item),
              );
        ...
        pythonProcess.on('error', (error) => {
          ...
          reject(
            new BadRequestException(`执行Python脚本失败: ${error.message}`),
          );
        });
      });
    } catch (error) {

...
export class EmployeeService {
  ...
  private readonly excludedEmployeeNames = ['曹海玲', '梁硕', '李然', '田帆'];
  ...
  private calculateWorkYears(hireDate: Date): number {
    ...
    if (
      ...
      (today.getMonth() === hireDateObj.getMonth() &&
        today.getDate() < hireDateObj.getDate())
    ) {
      yearsDiff--;
    }
  ...
  private isExcludedEmployee(name: string): boolean {
    ...
    this.logger.debug(
      `检查员工 "${trimmedName}" 是否为特定人员: ${isExcluded} (精确匹配: ${isExactMatch}, 在排除列表: ${isInExcludedList})`,
    );
    return isExcluded;
  }
  ...
  private getFixedWorkYears(name: string): number | null {
    ...
    const trimmedName = name.trim();
    for (const [employeeName, workYears] of Object.entries(
      this.fixedWorkYears,
    )) {
    ...
    if (this.excludedEmployeeNames.includes(trimmedName)) {
      this.logger.debug(
        `特定人员 "${trimmedName}" 在排除列表中，但没有设置固定工龄`,
      );
      return null;
    }
  ...
  private enforceWorkYearsLimit(employee: Employee): Employee {
    ...
    if (this.isExcludedEmployee(employee.name)) {
      ...
      if (fixedWorkYears !== null) {
        this.logger.debug(
          `使用特定人员 "${employee.name}" 的固定工龄: ${fixedWorkYears}`,
        );
        employee.workYears = fixedWorkYears;
      } else {
        this.logger.debug(
          `特定人员 "${employee.name}" 保持原工龄不变: ${employee.workYears}`,
        );
      }
      return employee;
    }
    ...
    if (employee.workYears > 5) {
      this.logger.debug(
        `限制员工 "${employee.name}" 的工龄: ${employee.workYears} -> 5`,
      );
  ...
  async create(createEmployeeDto: CreateEmployeeDto): Promise<Employee> {
    ...
    if (createEmployeeDto.idCardNumber) {
      ...
      if (existingEmployee) {
        ...
        throw new ConflictException(
          `身份证号 ${createEmployeeDto.idCardNumber} 已存在，不能重复使用`,
        );
      }
    }
    ...
    if (createEmployeeDto.resume) {
      this.logger.log(
        `收到的resume数据: ${JSON.stringify(createEmployeeDto.resume)}`,
      );
    }
    ...
    if (this.isExcludedEmployee(employee.name)) {
      ...
      if (fixedWorkYears !== null) {
        employee.workYears = fixedWorkYears;
        this.logger.log(
          `设置特定人员 "${employee.name}" 的固定工龄: ${fixedWorkYears}`,
        );
      }
    }
    // 如果是普通员工且提供了入职时间，自动计算公司工龄
      ...
      this.logger.log(
        `自动计算员工 "${employee.name}" 的工龄: ${employee.workYears}年`,
      );
    }
    ...
    if (savedEmployee.resume) {
      this.logger.log(
        `保存后的resume数据: ${JSON.stringify(savedEmployee.resume)}`,
      );
    }
  ...
  async findAll(queryEmployeeDto: QueryEmployeeDto) {
    ...
    for (let i = 0; i < employees.length; i++) {
      if (employees[i].workYears !== processedEmployees[i].workYears) {
        ...
        this.logger.log(
          `修正员工 "${employees[i].name}" 的工龄: ${employees[i].workYears} -> ${processedEmployees[i].workYears}`,
        );
      }
    }
  ...
  async findOne(id: number): Promise<Employee> {
    ...
    if (employee.workYears !== processedEmployee.workYears) {
      ...
      this.logger.log(
        `修正员工 "${employee.name}" 的工龄: ${employee.workYears} -> ${processedEmployee.workYears}`,
      );
    }
  ...
  async update(
    ...
    this.logger.log(
      `开始更新员工 ${id} - 名字: "${employee.name}", 当前工龄: ${employee.workYears}`,
    );

    // 检查是否更新了离职状态为已离职
    if (
      updateEmployeeDto.isResigned === true &&
      employee.isResigned === false
    ) {
      this.logger.log(
        `检测到员工 "${employee.name}" 状态变更为离职，将同步禁用其用户账号`,
      );
      try {
        // 查找关联的用户账号（通过员工身份证号匹配用户身份证号）
        if (employee.idCardNumber) {
          const user = await this.usersService.findByIdCardNumber(
          ...
          if (user) {
            ...
            this.logger.log(
              `已禁用员工 "${employee.name}" (身份证号: ${employee.idCardNumber}) 对应的用户账号`,
            );
          } else {
            this.logger.warn(
              `未找到员工 "${employee.name}" (身份证号: ${employee.idCardNumber}) 对应的用户账号`,
            );
          }
        } else {
          this.logger.warn(
            `员工 "${employee.name}" 没有身份证号，无法匹配用户账号`,
          );
        }
      } catch (error) {
        this.logger.error(
          `禁用员工 "${employee.name}" 对应的用户账号失败: ${error.message}`,
        );
      }
    }
    ...
    // 检查是否更新了离职状态为在职
    if (
      updateEmployeeDto.isResigned === false &&
      employee.isResigned === true
    ) {
      this.logger.log(
        `检测到员工 "${employee.name}" 状态变更为在职，将同步启用其用户账号`,
      );
      try {
        // 查找关联的用户账号（通过员工身份证号匹配用户身份证号）
        if (employee.idCardNumber) {
          ...
          if (user) {
            ...
            this.logger.log(
              `已启用员工 "${employee.name}" (身份证号: ${employee.idCardNumber}) 对应的用户账号`,
            );
          } else {
            this.logger.warn(
              `未找到员工 "${employee.name}" (身份证号: ${employee.idCardNumber}) 对应的用户账号`,
        ...
        this.logger.error(
          `启用员工 "${employee.name}" 对应的用户账号失败: ${error.message}`,
        );
      }
    }
    ...
    this.logger.log(
      `特定人员检查 - 员工: "${employee.name}", 结果: ${isExcluded}`,
    );
    ...
    if (isExcluded) {
      ...
      const fixedWorkYears = this.getFixedWorkYears(employee.name);
      this.logger.log(
        `特定人员 "${employee.name}" 的固定工龄: ${fixedWorkYears}`,
      );

      // 如果工龄值发生了变化，先直接通过SQL更新回固定值
      if (fixedWorkYears !== null && employee.workYears !== fixedWorkYears) {
        this.logger.log(
          `特定人员 "${employee.name}" 的工龄已被修改，正在恢复为固定值: ${employee.workYears} -> ${fixedWorkYears}`,
        );
    ...
    if (updateEmployeeDto['idCardNumber'] !== undefined) {
      this.logger.warn(
        `尝试修改身份证号码: ${employee.idCardNumber} -> ${updateEmployeeDto['idCardNumber']}`,
      );
    ...
    if (updateEmployeeDto.resume) {
      this.logger.log(
        `更新resume数据: ${JSON.stringify(updateEmployeeDto.resume)}`,
      );
    }
    ...
    if (updateEmployeeDto.hireDate) {
      this.logger.log(
        `正在更新入职时间: ${oldHireDate} -> ${updateEmployeeDto.hireDate}`,
      );
    }
    ...
    if (updateEmployeeDto.workYears !== undefined && isExcluded) {
      this.logger.warn(
        `尝试修改特定人员 "${employee.name}" 的工龄: ${originalWorkYears} -> ${updateEmployeeDto.workYears}，已阻止`,
      );
    ...
    if (
      updateEmployeeDto.rank &&
      (employee.position === '记账会计' ||
        updateEmployeeDto.position === '记账会计')
    ) {
      this.logger.log(
        `检测到rank字段更新且position为记账会计，rank值: ${updateEmployeeDto.rank}`,
      );
      ...
      if (rankMatch) {
        ...
        this.logger.log(
          `解析rank值: ${updateEmployeeDto.rank}, pLevel=${pLevel}, gradeLevel=${gradeLevel}`,
        );

        try {
          // 查询匹配的绩效提成记录
          const performanceCommission =
            await this.performanceCommissionRepository.findOne({
              where: {
                pLevel: pLevel,
          ...
          if (
            ...
            performanceCommission.baseSalary !== undefined
          ) {
            this.logger.log(
              `找到匹配的绩效提成记录，自动更新baseSalary: ${originalBaseSalary || 0} -> ${performanceCommission.baseSalary}`,
            );
            updateData.baseSalary = performanceCommission.baseSalary;
          } else {
            this.logger.warn(
              `未找到匹配的绩效提成记录: pLevel=${pLevel}, gradeLevel=${gradeLevel}`,
            );
          }
        } catch (error) {
        ...
        this.logger.warn(
          `rank字段格式不正确，无法自动更新baseSalary: ${updateEmployeeDto.rank}`,
        );
      }
    }
    ...
    if (isExcluded) {
      ...
      if (fixedWorkYears !== null) {
        employee.workYears = fixedWorkYears;
        this.logger.debug(
          `强制设置特定人员 "${employee.name}" 的工龄为固定值: ${fixedWorkYears}`,
        );
      } else {
        employee.workYears = originalWorkYears;
        this.logger.debug(
          `保持特定人员 "${employee.name}" 的工龄不变: ${originalWorkYears}`,
        );
      }
    } else if (
      employee.hireDate &&
      (updateEmployeeDto.hireDate || !oldHireDate)
    ) {
      ...
      this.logger.debug(
        `更新普通员工 "${employee.name}" 的工龄: ${originalWorkYears || 0} -> ${calculatedWorkYears}`,
      );
    }
    ...
    // 打印最终结果
    this.logger.debug(
      `最终更新结果 - 员工: "${employee.name}", 工龄: ${employee.workYears}年`,
    );
    ...
    if (
      ...
      updateData.baseSalary !== originalBaseSalary
    ) {
      // 获取当前用户名（修改人）
      let modifiedBy = '系统';
      ...
      this.logger.log(
        `已记录员工 "${employee.name}" 的底薪变更历史: ${originalBaseSalary || 0} -> ${updateData.baseSalary}, 修改人: ${modifiedBy}`,
      );
    }
    ...
    try {
      ...
      for (const employee of employees) {
        ...
        if (this.isExcludedEmployee(employee.name)) {
          ...
          if (
            ...
            await this.employeeRepository.update(employee.id, {
              workYears: fixedWorkYears,
            });
            this.logger.debug(
              `修正特定人员 "${employee.name}" 的工龄: ${originalWorkYears} -> ${fixedWorkYears} (固定值)`,
            );
          } else {
            this.logger.debug(
              `跳过特定人员 "${employee.name}" 的工龄更新，保持原值: ${employee.workYears}`,
            );
          }
          continue;
        }
        ...
        if (employee.hireDate) {
          const calculatedWorkYears = this.calculateWorkYears(
            employee.hireDate,
          );
          ...
          if (calculatedWorkYears !== employee.workYears) {
            employee.workYears = calculatedWorkYears;
            await this.employeeRepository.update(employee.id, {
              workYears: calculatedWorkYears,
            });
            this.logger.debug(
              `修正普通员工 "${employee.name}" 的工龄: ${originalWorkYears || 0} -> ${calculatedWorkYears}`,
            );
          }
        }
      }

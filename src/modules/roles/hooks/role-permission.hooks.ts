...
@Injectable()
export class RolePermissionHooks
  implements OnModuleInit, EntitySubscriberInterface<Role>
{
  ...
  async afterInsert(event: InsertEvent<Role>): Promise<void> {
    ...
    try {
      ...
      // 首先尝试从数据库中获取所有唯一权限
      const uniquePermissions = await manager
        ...
        .distinct(true)
        .groupBy(
          'permission.page_name, permission.permission_name, permission.description',
        )
        .getRawMany();
      ...
      this.logger.log(
        `已为角色 "${role.name}" 创建 ${permissions.length} 个权限记录`,
      );
    } catch (error) {
      this.logger.error(
        `为角色 "${role.name}" 创建权限记录时出错: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async afterUpdate(event: UpdateEvent<Role>): Promise<void> {
    if (
      !event.entity ||
      !event.updatedColumns.find((col) => col.propertyName === 'name')
    ) {
      return;
    }

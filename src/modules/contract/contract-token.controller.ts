...
export class ContractTokenController {
  ...
  async generateTemporaryToken(@Query('id') id: string) {
    ...
    try {
      ...
      try {
        ...
        const token = await this.tokenService.generateTemporaryToken(
          contractId,
          10080,
        );
        ...
        if (error instanceof ForbiddenException) {
          this.logger.warn(
            `使用默认管理员ID查询合同失败，合同ID: ${contractId}`,
          );
  ...
  async getContractImage(@Query('token') token: string) {
    ...
    try {
      // 验证令牌有效性并获取合同图片和合同类型
      const contractInfo =
        await this.tokenService.getContractImageByToken(token);
  ...
  private getSignaturePosition(contractType: string): {
    x: number;
    y: number;
    width: number;
  } {
  ...
  private getContractImageUrl(contractImage: string): string {
    ...
    return `${protocol}://${minioEndpoint}:${minioPort}/${minioBucketName}/${contractImage}`;
  }
  ...
  private async mergeImages(
    ...
    try {
      ...
      console.log(
        `===== 获取到的签名位置: x=${position.x}, y=${position.y}, width=${position.width} =====`,
      );
      this.logger.log(
        `获取到的签名位置: x=${position.x}, y=${position.y}, width=${position.width}`,
      );
      ...
      console.log(
        `===== 签名图片原始尺寸: ${originalWidth}x${originalHeight}, 缩放后尺寸: ${position.width}x${resizedHeight} =====`,
      );
      this.logger.log(
        `签名图片原始尺寸: ${originalWidth}x${originalHeight}, 缩放后尺寸: ${position.width}x${resizedHeight}`,
      );
      ...
      console.log(
        `===== 签名位置: x=${position.x}, y=${position.y}, 转换后top=${topPosition} =====`,
      );
      this.logger.log(
        `签名位置: x=${position.x}, y=${position.y}, 转换后top=${topPosition}`,
      );
  ...
  async saveSignature(@Body() saveSignatureDto: SaveSignatureDto) {
    ...
    console.log(
      `接收到签名请求，合同ID: ${contractId}, 签名URL: ${signatureUrl}`,
    );
    this.logger.log(
      `接收到签名请求，合同ID: ${contractId}, 签名URL: ${signatureUrl}`,
    );
    ...
    // 验证token是否与合同ID相关联
    const isValid = await this.tokenService.validateTokenForContract(
      token,
      contractId,
    );
    if (!isValid) {
    ...
    try {
      ...
      // 2. 构建合同图片URL并下载
      const contractImageUrl = this.getContractImageUrl(
        contractInfo.contractImage,
      );
      ...
      const mergedImagePath = path.join(
        os.tmpdir(),
        'contract-images',
        mergedImageName,
      );
      ...
      const success = await this.contractService.saveContractSignature(
        contractId,
        signatureUrl,
      );
      if (!success) {
      ...
      const updateSuccess = await this.contractService.updateContractImage(
        contractId,
        uploadedFileName,
      );
      if (!updateSuccess) {
      ...
      // 签名保存成功后，获取合同的加密编号
      const encryptedCode =
        await this.contractService.getContractEncryptedCode(contractId);
      ...
      try {
        ...
        const result = await this.contractService['contractRepository'].query(
          query,
          [contractId],
        );

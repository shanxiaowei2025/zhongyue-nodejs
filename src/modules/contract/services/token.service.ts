...
export class TokenService {
   ...
   */
  async generateTemporaryToken(contractId: number, expiresInMinutes: number = 30): Promise<Token> {
    // 检查合同状态
    ...
    if (contractStatus === '1' || contractStatus === '2') {
      ...
      this.logger.warn(`合同 #${contractId} ${statusMessage}，删除所有相关令牌并拒绝生成新令牌`);
    ...
    this.logger.debug(`为合同 #${contractId} 生成临时令牌，过期时间: ${expiredAt.toISOString()}`);
  ...
  async validateToken(token: string): Promise<Token | null> {
    ...
    if (tokenEntity.expiredAt && new Date() > tokenEntity.expiredAt) {
      this.logger.debug(`令牌已过期: ${token}, 过期时间: ${tokenEntity.expiredAt.toISOString()}`);
      // 删除过期令牌
    ...
    // 检查合同是否已签署或已终止
    if (tokenEntity.contract && 
        (tokenEntity.contract.contractStatus === '1' || tokenEntity.contract.contractStatus === '2')) {
      const statusMessage = tokenEntity.contract.contractStatus === '1' ? '已签署' : '已终止';
      this.logger.debug(`合同 #${tokenEntity.contractId} ${statusMessage}，令牌 ${token} 无效`);
      // 删除该合同的所有令牌
  ...
  async deleteContractTokens(contractId: number): Promise<void> {
    ...
    this.logger.debug(`删除合同 #${contractId} 的所有令牌, 删除数量: ${result.affected || 0}`);
  }
   ...
   * @param contractId 合同ID
   */
  async validateTokenForContract(token: string, contractId: number): Promise<boolean> {
    try {
      const tokenEntity = await this.tokenRepository.findOne({
        where: { 
          token,
      ...
      if (contractStatus === '1' || contractStatus === '2') {
        const statusMessage = contractStatus === '1' ? '已签署' : '已终止';
        this.logger.debug(`合同 #${contractId} ${statusMessage}，令牌 ${token} 无效`);
        // 删除该合同的所有令牌
        await this.deleteContractTokens(contractId);
        return false;
      }

...
export class ContractService {
  ...
  private async checkAndUpdateExpiredContracts(
    ...
    if (contractsToUpdate.length > 0) {
      this.logger.log(
        `发现 ${contractsToUpdate.length} 个已签署且委托已到期的合同，更新状态为已终止`,
      );
  ...
  async findAll(
    ...
    const addConditions = (conditions: any) => {
      ...
      if (query.partyASignDateStart && query.partyASignDateEnd) {
        ...
        this.logger.debug(
          `甲方签订日期查询范围: ${startDate.toISOString()} - ${endDate.toISOString()}`,
        );
      ...
      if (query.createTimeStart && query.createTimeEnd) {
        ...
        this.logger.debug(
          `合同查询日期范围: ${startDate.toISOString()} - ${endDate.toISOString()}`,
        );

...
export class ContractController {
  ...
  async create(@Body() createContractDto: CreateContractDto, @Request() req) {
    this.logger.debug(
      `用户请求创建合同，用户信息: ${JSON.stringify(req.user)}`,
    );
  ...
  @ApiOperation({
    summary: '获取合同列表',
    description:
      '通过各种条件筛选合同列表。使用createTimeStart和createTimeEnd参数可以按创建日期范围筛选，partyASignDateStart和partyASignDateEnd参数可以按甲方签订日期范围筛选。如果使用相同的日期（如createTimeStart=2023-01-01&createTimeEnd=2023-01-01），查询将返回该整天的数据。',
  })
  @ApiResponse({ status: 200, description: '获取合同列表成功' })
  @ApiResponse({ status: 403, description: '没有查看合同的权限' })
  async findAll(@Query() query: QueryContractDto, @Request() req) {
    if (!req.user || !req.user.id) {
      throw new ForbiddenException('未能获取有效的用户身份');
    }

    this.logger.debug(
      `用户 ${req.user.username} (ID: ${req.user.id}) 请求获取合同列表，角色: ${req.user.roles?.join(', ')}`,
    );
  ...
  async signContract(
    ...
    const result = await this.contractService.signContract(
      +id,
      signContractDto,
      req.user.id,
      req.user.username,
  ...
  @UseGuards(JwtAuthGuard, RolesGuard) // 使用JWT认证和角色守卫
  @ApiOperation({
    summary: '根据企业名称或统一社会信用代码获取代理记账合同日期',
  })
  @ApiResponse({ status: 200, description: '获取代理日期成功' })
  @ApiResponse({ status: 400, description: '请求参数错误' })
  @ApiResponse({ status: 404, description: '未找到代理记账合同记录' })
  @ApiQuery({
    name: 'companyName',
    required: false,
    description: '企业名称',
  ...
  async getAgencyDates(@Query() query: GetAgencyDatesDto, @Request() req) {
    if (!req.user || !req.user.id) {
      throw new ForbiddenException('未能获取有效的用户身份');
    }

    this.logger.debug(
      `用户请求获取代理日期，原始查询参数: ${JSON.stringify(query)}`,
    );
    ...
    try {
      ...
      if (query.companyName) {
        try {
          const trimmedCompanyName = decodeURIComponent(
            query.companyName.trim(),
          );
          if (
            trimmedCompanyName &&
            trimmedCompanyName !== 'NaN' &&
      ...
      if (query.unifiedSocialCreditCode) {
        try {
          const trimmedCreditCode = decodeURIComponent(
            query.unifiedSocialCreditCode.trim(),
          );
          if (
            trimmedCreditCode &&
            trimmedCreditCode !== 'NaN' &&
            trimmedCreditCode !== 'undefined'
          ) {
      ...
      this.logger.debug(
        `处理后的查询参数: companyName='${companyName}', unifiedSocialCreditCode='${unifiedSocialCreditCode}'`,
      );

      const result = await this.contractService.getAgencyDates(
        companyName,

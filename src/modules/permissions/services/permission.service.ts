...
//   }
  ...
  async findAll(query?: GetPermissionsQueryDto): Promise<Permission[]> {
    ...
    if (query?.page_name) {
      queryBuilder.andWhere('permission.page_name = :pageName', { pageName: query.page_name });
    }
...
//   }
  ...
  async getUniquePermissions(): Promise<Array<{ page_name: string; permission_name: string; description: string; }>> {
   ...
   * @returns 权限值布尔值和权限信息
   */
  async getPermissionValueByName(role_name: string, permission_name: string): Promise<{
    permission_value: boolean;
    permission?: Permission;
  }> {
    try {
      console.log(`查询权限: role_name=${role_name}, permission_name=${permission_name}`); // 添加日志
      ...
      const permission = await this.permissionRepository
        ...
        .andWhere('permission.permission_name = :permName', { permName: permission_name })
        .getOne();
      ...
      console.log(`找到权限记录: id=${permission.id}, value=${permission.permission_value}`);
      return { 
